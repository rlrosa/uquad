...
%      sin(z)*cos(y) , cos(z)*cos(x) + sin(z)*sin(y)*sin(z)  , cos(z)*sin(x) - sin(z)*sin(y)*cos(x)  ; ...
%      sin(y)        , -cos(y)*sin(x)                        , cos(y)*cos(x)];
R_1 = [cos(z)*cos(y); sin(z)*cos(y); sin(y)];
R_2 = [-sin(z)*cos(x) + cos(z)*sin(y)*sin(x); cos(z)*cos(x) + sin(z)*sin(y)*sin(z); -cos(y)*sin(x)];
R_3 = [-sin(z)*sin(x) - cos(z)*sin(y)*cos(x); cos(z)*sin(x) - sin(z)*sin(y)*cos(x); cos(y)*cos(x)];
R = [R_1, R_2, R_3];

% Call with gyro data: (x_0,y_0,z_0)
eval_R_3_0 = @(x,y,z) [ -sin(z)*sin(x) - cos(z)*sin(y)*cos(x) ; ...
...
% MatLab does the math:
A = [ cos(z)*sin(x)*sin(y) - cos(x)*sin(z)  , -cos(x)*cos(y)*cos(z),  cos(x)*sin(y)*sin(z) - cos(z)*sin(x)    ; ...
      cos(x)*cos(z) + sin(x)*sin(y)*sin(z)  , -cos(x)*cos(y)*sin(z),  - sin(x)*sin(z) - cos(x)*cos(z)*sin(y)  ; ...
      -cos(y)*sin(x)                        ,        -cos(x)*sin(y),  0                                       ];

b = [ ax + sin(x_0)*sin(z_0) - x_0*(cos(x)*sin(z) - cos(z)*sin(x)*sin(y)) - z_0*(cos(z)*sin(x) - cos(x)*sin(y)*sin(z)) + cos(x_0)*cos(z_0)*sin(y_0) - y_0*cos(x)*cos(y)*cos(z) ; ...
