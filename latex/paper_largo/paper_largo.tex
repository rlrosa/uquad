...
\begin{abstract}
%\boldmath
This paper describes the design and integration of an instrumentation, sensor fusion and control system that allows the autonomous flight of a quadrotor. A comercial frame is used, a mathematical model for the quadrotor is developed and its parameters determined from the characterization of the unit. An intelligence is integrated to play the role of the flight controller. A 9 degrees of freedom Inertial Measurement Unit (IMU) equipped with a barometer is calibrated and added to the platform. Sensor fusion is done by two modified Extended Kalman Filters (EKF): one combining data provided by IMU and the other also including the information provided by GPS. A reliable estimation of the state variables is obtained. Three states representing systematic bias in the accelerometer measurements are also added to the EKF, which improves the inertial estimation of the position. The control actions are obtained from a proportional-integral controller based on an improved Linear Quadratic Regulator (LQR) algorithm. A stable autonomous platform is achieved.%For windless environments, a stable platform is achieved.
% -cambio-
\end{abstract}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item $ \mathbf{P_{G}}_{k|k-1} =  {{\mathbf{F_{G}}_{k-1}}} \mathbf{P_{G}}_{k-1|k-1}{  {\mathbf{F_{G}}_{k-1}^T}} + \mathbf{Q_{G}}_{k-1} $
	\end{itemize}
	\item Update
	\begin{itemize}
		...
		\item $ \mathbf{P_{G}}_{k|k} = (I - \mathbf{K_{G}}_{k} { \mathbf{H_{G}}_{k}}) \mathbf{P}_{k|k-1} $
	\end{itemize}
\end{itemize}
...
\begin{figure}
  \centering
  \subfloat[\textbf{General diagram}]{\label{fig:diagrama_kalman1} 
	\includegraphics[width=1\columnwidth] {./pics_paper/diagrama_kalman_eng.png}} \\
  \subfloat[\textbf{Double EKF -} Detail.]{\label{fig:double_ekf} 
	\includegraphics[width=0.6\columnwidth] {./pics_paper/double_ekf.png}}
...
%Just before the quadrotor takes off a final calibration is implemented assuming the quadrotor is absolutely still. In this situation the accelerometer must indicates $a=g$.\\

\begin{figure}
  \centering
  \subfloat[Without bias estimation]{\label{fig:sin_bias} \includegraphics[width=0.5\columnwidth] {./pics_paper/sin_bias.pdf}} \hspace*{-5pt}
  \subfloat[With bias estimation]{\label{fig:con_bias} \includegraphics[width=0.5\columnwidth] {./pics_paper/con_bias.pdf}}
...
\begin{equation}
\label{eq:lqr}
\int_{0}^{\infty}  (\mathbf{x}-\mathbf{x}_{sp})^T\mathbf{Q} (\mathbf{x}-\mathbf{x}_{sp})+(\mathbf{u}-\mathbf{u}_{sp})^T\mathbf{R}(\mathbf{u}-\mathbf{u}_{sp})dt
\end{equation}

where $\mathbf{Q}$ and $\mathbf{R}$ are positive-definite matrices, weighting the energy of the departure from the objective state and the objective control signal respectively. The choice of $\mathbf{Q}$ and $\mathbf{R}$ is a trade-off between the error that is tolerated and the energy that the system can use to correct it. In equation \refp{eq:lqr} $\mathbf{x}_{sp}$ is the objective state and $\mathbf{u}_{sp}$ is the theoretical control action that is compatible with the state $\mathbf{x}_{sp}$. Both vectors as well as $\mathbf{x}$ and $\mathbf{u}$ are time dependent. 
...
\begin{equation}
\begin{array}{c}
\mathbf{u}(t)-\mathbf{u}_{sp}(t) = -K(\mathbf{x}(t)-\mathbf{x}_{sp}(t))\\
\\
...
where $\mathbf{P}$ is the solution to Riccati's equation:
\begin{equation}
  \label{eq:riccati}
...
In this work an integral effect is added over the LQR controller. For this, an expanded state vector is defined $[\mathbf{x}^T, \mathbf{x}_I^T]^T$. For the controllability and observability hypothesis to hold, only some state variables may be passed through the integral control block: $\mathbf{\dot{x}}_I =[x,y,z,\theta]^T$. After making this definitions, the system is re-written in state variables form. The method to obtain the new matrix is the same that explained in Section \refp{sec:lqr}
\section{Software}
