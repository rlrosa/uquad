...
El \textit{omap} cuenta con 4 módulos para \textit{PWM} implementados en hardware. Para poder utilizarlos es necesario deshabilitar una función de ahorro de energía en el kernel. Se utilizó \href{http://www.openembedded.org}{OpenEmbedded} para compilar un kernel adecuado.
...
Solamente 3 de los 4 \textit{PWM} están mapeados a pines accesibles en el \textit{BB}. Para tener acceso al cuarto \textit{PWM} es necesario modificar la configuración de la multiplexación en el \textit{omap}.

Habilitar el cuarto \textit{PWM} trae al menos dos problemas:
\begin{enumerate}
...
El microcontrolador a bordo del \textit{IMU} tiene un \textit{ADC} de 10 bits, a cuya entrada están conectadas, mediante multiplexación, las salidas de los sensores: 3 giróscopos y 3 acelerómetros\footnote{En realidad es 1 acelerómetro de 3 ejes.}.

El software que corre en el el \textit{IMU} permite configurar qué sensores se desean leer, y la frecuencia con la que se debe realizar el muestreo. Las lecturas se transmiten mediante una \textit{UART}. Esta información se puede ver en una PC utilizando un adaptador \textit{USB-serie}, o se pueden recibir en el \textit{BB} conectando el \textit{IMU} a alguna de las \textit{UART}s en el \textit{BB}.
...
El código que corre en el \textit{IMU} permite elegir entre un formate \textit{ASCII}, o formato binario.

La información transmitida por el \textit{IMU} es de la forma:
...
Donde:
\begin{enumerate}
\item \verb+A+: Indica comienzo de datos - 2 bytes.
\item \verb+count+: Cada tira de datos tiene un número asociado - 4 bytes.
\item \verb+acc*+: Lectura del \textit{ADC} para los acelerómetros - 2 bytes cada uno.
\item \verb+pitch roll yaw+: Lectura del \textit{ADC} para los giróscopos - 2 bytes cada uno.
\item \verb+Z+: Indica fin de datos - 2 bytes.
\end{enumerate}

En modo binario solamente se transmiten los campos previamente mecionados, mientras que en modo \textit{ASCII}:
\begin{enumerate}
\item Cada dígito se transmite como un \verb+char+, ocupando 1 byte. Esto implica que un valor de \verb+count+ de $24662$, cuya representación en hexadecimal es $0x6056$ ocupará 5 bytes, en lugar de 4.
\item Se utilizan tabulaciones para separar cada campo.
\end{enumerate}

El modo \textit{ASCII} es más sencillo, pero el modo binario es más eficiente. Se optó por utilizar el modo \textit{ASCII} para pruebas, y el modo binario para la implementación que correrá en el \textit{BB}.
