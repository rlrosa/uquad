...
A quadrotor platform can be imagined performing countless tasks, such as search and rescue, wild fire suppression, or scientific research\cite{bib:apps}. Many applications are being developed based on such platforms, but how do they fly? An aerial vehicle is inherently unstable, staying still is not a simple task. This paper focuses on the stabilization of a quadrotor, explaining the development of the mathematical model to represent the system, the filtering techniques applied for sensor data fusion and the control system that allows the quadrotor to fly.
% -cambio-
...
The mathematical model developed is similar to others presented in the literature \cite{bib:quadrotor-bible}, \cite{bib:curso_quad}. However, in the named literature, the center of gravity of the quadrotor is assumed to be at the same height that the propellers. To obtain proper results, a momentum produce by the gravity force had to be added. Annother difference that between the model developed and the model from \cite{bib:curso_quad} is that in this paper the linear velocities are relatives to the quadrotor frame, while in the literature are expresed in an inertial frame.  
...
%The gyro drifts over time; the accelerometer is very sensible to the vibrations generated by the motors; the magnetometer is distorted by ferromagnetic materials; the GPS has a very poor accuracy and a slow update rate. Each sensor by itself is very limited,
...
In this paper an integral effect is added over the LQR controller. For this, an expanded state vector is defined $[X^T, X_I^T]^T$. For the controllability and observability hypothesis to hold, only some state variables may be passed through the integral control block: $\dot{X}_I =[x,y,z,\theta]^T$. After making this definitions, the system is re-written in state variables form. The method to obtain the new matrix is the same that explained in \ref{sec:lqr}
\section{Software}
...
During this first stage, the quadrotor was only allowed to move (and correct) one of the following Euler angles: $\{\psi,\phi\}$. The control system was tuned until an acceptable behavior was obtained. This procedure was applied to each angle. Once this stage was completed the quadrotor was able to maintain horizontality by controlling $\{\psi,\phi,\omega_{q_x},\omega_{q_y}\}$. The behavior for the Roll ($\psi$) angle is shown in figure \refp{fig:psimetro}. While in figure \refp{fig:psimetro_psi} is shown the mechanical perturbation imposed to the \emph{Roll} angle, the reaction can be observed in figure \refp{fig:psimetro_w}. From $t_0 = 80.9 s$ to $t_1 =81.3 s$ the perturbation is present. In figure \ref{fig:psimetro_w} the response of the controller is adecuate, yet inefficient because the model considered did not take account of this kind of effect. After this, the controller is working freely. The system step response for a step of more than $20^o$ shows a overshoot of $3^o$ or $4^o$ and a rise time of about $0.4\;s$, which means an excellent performance, as can be seen in figure \refp{fig:psimetro_psi}.
\begin{figure}[h!]
  \centering
  \subfloat[Roll angle deviation]{\label{fig:psimetro_psi}
  		\includegraphics[width=0.5\columnwidth]
  			{./pics_paper/psimetro_psi.pdf}} \hspace*{-5pt}
  \subfloat[Difference between $\omega$ and $\omega_{hover}$ for each motor]{\label{fig:psimetro_w} 
  		\includegraphics[width=0.5\columnwidth]
  			{./pics_paper/psimetro_w.pdf}}
...
\begin{figure}[h!]
  \centering
  \subfloat[Yaw angle deviation]{\label{fig:theta}
  		\includegraphics[width=0.5\columnwidth]
  			{./pics_paper/theta.pdf}} \hspace*{-5pt}
  \subfloat[Difference between $\omega$ and $\omega_{hover}$ for each pair of motors.]{\label{fig:theta_w} 
  		\includegraphics[width=0.5\columnwidth]
  			{./pics_paper/theta_w.pdf}}
...
%\textbf{Free flight}: The last step was to let the quadrotor fly. This stage required the GPS. After it was completed, the control system was able to hold it's horizontal position $\{x,y,v_{q_x},v_{q_y}\}$, completing control over the state variables.
...
%To ilustrate some results, figure \refp{fig:psi_esc.pdf} shows the step response for $\psi$ during an experiment where this was the only degree of freedom. 
...
%\begin{figure}
%	\centering
%	\includegraphics[width=.8\columnwidth]{./pics_paper/control_escalon_eng.png}
...
The two main goals were successfuly achieved. First, the design and implementation of the sensor fusion was accomplished by the integration of all the measurements in an Extended Kalman Filter which is used for denoising purposes without introducing delay. In addition a basic controller, capable of stabilizing the system, was implemented based on LQR algorithm. A simplified dynamic model was derived as a first step for both Kalman Filter and controller. \\
...
The controller was successfully tested for attitude and height. Due to lack of precision of mentioned velocities this information is not used. However, several simulations were made assuming that a proper velocity estimation is available and the designed controller proved to work properly. With a similar controller the quadrotor is able to perform some other trajectories. However, if a nonlinear controller is implemented the possibilities are virtually infinity.\\

allowing the quadrotor to perform tasks such as power line inspection, surveillance, assistance during natural disasters, etc.

The doors are open for future projects to add additional levels of intelligence, however it will be necessary to perform a major improvement. The choice of implementing the flight controller on an operating system that does not run in real-time made timing a complicated issue. These issues were overcome, yet assigning additional tasks to the system will surely disrupt timing. Porting the stabilization system, which requires a very fast and steady loop, to a dedicated microprocessor would be a significant improvement. It would allow the code running on top of the operating system to perform higher level tasks, such as Visual Simultaneous Localization and Mapping (VSLAM), navigation, tracking, power line inspection, surveillance, assistance during natural disasters, etc.
