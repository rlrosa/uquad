...
The platform is based on a commercial quadcopter, the frame and the motor control system were preserved, but the IMU and intelligence were replaced by the flight controller that was developed. An ARM-Cortex-A8 processor running Linux performs the computations requiered to convert raw IMU data and combine it using an EKF, overcoming the noise generated by the vibrations of the motors, and providing a reliable estimation of the state variables. Once the current state is known, the LQR algorithm is used to derive control actions required to bring the system to the desired setpoint.
...
The theory behind a standard Kalman Filter does not hold for a system that is not linear. The model for the quadcopter is highly non-linear, so an EKF is implemented based on \cite{bib:kalman} and \cite{bib:kalman2}. An EKF is not optimal, and the performance is highly dependant on the linearizacion \cite{bib:kay}. The performance achieved is considered acceptable.
