...
The platform is based on a commercial quadcopter, the frame and the motor control system were preserved, but the IMU and intelligence were replaced by the flight controller that was developed. An ARM-Cortex-A8 processor running Linux performs the computations requiered to convert raw IMU data and combine it using an EKF, overcoming the noise generated by the vibrations of the motors, and providing a reliable estimation of the state variables. Once the current state is known, the LQR algorithm\cite{bib:lqrnotes}\cite{bib:lqr-discreto} is used to derive control actions required to bring the system to the desired setpoint.
...
\begin{figure}
	\centering
	\includegraphics[width=1\columnwidth]{./pics_paper/quad_coord.jpg}
	\caption{Model of a quadcopter - Intertial reference (blue), Non-Inertial reference (red)}
	\label{fig:quad}
\end{figure}
...
The equations that govern the dynamics of a quadcopter are presented in equation \ref{eq:modelo}. The reference frames used are shown in figure \ref{}.
