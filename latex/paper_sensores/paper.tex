...
In the datasheets of the considered sensors(\cite{bib:acc_data} and \cite{bib:gyro_data}) the effect of nonlinearityis negligible since it represents the $\pm 0.5.\%$  of the full scale for the accelerometer and the $\pm 0.2\%$ for the gyroscope. Therefore we will not discuss any further on this issue.\\
%TODO Esto es medio turbio que sea despreciable... en realidad me da que no habr\'ia que despreciarlo. Capaz que es mejor esconder este punto.

In figure \ref{fig:ruido} we can observe that the electrical noise of the devices is typically 2 bits, this error (acording the datashets \cite{bib:acc_data} and \cite{bib:gyro_data})correspond to $7.8 mg$ and $0.14 ^{\circ}/s $.  
%TODO Cuando tengamos la gráfica ver bien cuanto es el ruido y a cuanto corresponde en la vida real. Tampoco se si es tan despreciable. Capaz que hay que decir que se usan muchas muestras normalmente y que variaciones aleatorias de cierta varianza no joden mucho. Pero ahora no me voy a poner a hacer eso xq tengo sueño. 
...
\subsubsection{Temperature adjust}
