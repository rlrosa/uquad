...
It is interesting to compare the values obtained with the theoretical values of each parameter. As we can see, comparing the vector obtained with the seed, the values of the gains and the rotations are similar to what is declared in the datasheets. Furthermore, the offset values are also consistant with the information of the datasheet, yet this is not a straightforward conclusion, thus we present the following analyisis. In \cite{bib:acc_data} is declared that the $0g$ output is $\pm 150 mg$ for axes $x$ and $y$ and $\pm 250mg$ for axis $z$. This correspond to offsets of $39$ bits maximum for $x$ and $y$, and $64$ for $z$. Therefore, we can conclude that the results obtained are consistent with what is declared in the datasheets. 
