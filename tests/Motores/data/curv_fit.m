...
Ftest_cuad = p_quad(1)*(wtest.^2)+p_quad(2)*wtest;

% MODELO CUBICO
B_cub     = [w.^3 w.^2 w];
p_cub     = (B_cub'*B_cub)\(B_cub'*Fm);
Ftest_cub = p_cub(1)*(wtest.^3)+p_cub(2)*(wtest.^2)+p_cub(3)*wtest;

% ERRORES
e_cuad      = Fm-(p_quad(1)*w.^2+p_quad(2)*w);
e_cuad_prom = mean(e_cuad);
sigma_cuad  = std(e_cuad);
fprintf('Modelo Cuadrático\n\tError: %d\n\tSigma: %d\n',e_cuad_prom,sigma_cuad)

e_cub       = Fm-(p_cub(1)*(w.^3)+p_cub(2)*(w.^2)+p_cub(3)*w);
